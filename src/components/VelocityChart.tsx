...
export default function VelocityChart({ vFrames, repStats }: Props) {
  // Build chart data — split smoothed into concentric/eccentric series
  // so Recharts can colour each phase independently
  ...
  // Rep boundary lines — find first frame of each rep
  const repBoundaries = repStats.map((s) => {
    const firstFrame = vFrames.find(f => f.repIndex === s.repNumber - 1 && f.phase !== "rest");
    return { repNumber: s.repNumber, time: firstFrame?.timeSeconds.toFixed(2) ?? "0" };
  });
  ...
  const CustomTooltip = ({ active, payload, label }: {
    ...
    return (
      <div className="bg-[#1a1a1a] border border-white/10 rounded-lg px-3 py-2 text-xs">
        ...
        <p className="font-bold" style={{ color: PHASE_COLOUR[point?.phase ?? "rest"] }}>
          {point?.phase ?? "rest"}
          {point?.repIndex !== null ? ` · Rep ${(point.repIndex ?? 0) + 1}` : ""}
        </p>
  ...
  return (
    <div className="bg-white/5 border border-white/10 rounded-xl p-6 flex flex-col gap-6">
      ...
      <ResponsiveContainer width="100%" height={280}>
        <ComposedChart data={data} margin={{ top: 4, right: 16, bottom: 16, left: 0 }}>
          ...
          <XAxis
            ...
            label={{ value: "Time (s)", position: "insideBottom", offset: -8, fill: "rgba(255,255,255,0.3)", fontSize: 11 }}
          />
          <YAxis
            stroke="rgba(255,255,255,0.2)"
            tick={{ fontSize: 10, fill: "rgba(255,255,255,0.3)" }}
            domain={[0, Math.ceil(maxV * 1.2)]}
            label={{ value: "px/s", angle: -90, position: "insideLeft", fill: "rgba(255,255,255,0.3)", fontSize: 11 }}
          />
      ...
      {/* Per-rep peak velocity mini-chart */}
      {repStats.length > 0 && (
              ...
              return (
                <div key={s.repNumber} className="flex flex-col items-center gap-1 flex-1">
                  ...
                  <div
                    ...
                    style={{ height: `${barH}%`, background: colour, opacity: 0.8 }}
                  />
